...
const PersonalChats = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        let chatRoomId = currentUserRandomNumber < User.randomNumber
            ? currentUserRandomNumber + User.randomNumber
            : User.randomNumber + currentUserRandomNumber;
    ...
    // Add a new function to handle media selection
    ...
    //             if (type === 'photo') {
    //                 onSend([{ key: Math.random().toString(), image: source.uri }]);
    //             } else if (type === 'video') {
    //                 // onSend([{ key: Math.random().toString(), video: source.uri }]);
    //                 const thumbnail = await generateVideoThumbnail(response.path);
    //                 onSend([{ key: Math.random().toString(), video: source.uri, videoThumbnail: thumbnail }]);
    //             } else if (type === 'audio') {
    //                 onSend([{ key: Math.random().toString(), audio: source.uri }]);
    //             }
    //         }
    //     });
    // };
    ...
    //             ...message,
    //             user: {
    //                 _id: currentUserRandomNumber, // This should be the current user's ID
    ...
    const onSend = useCallback((newMessages = []) => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [userData, currentUserRandomNumber]);
    ...
    const sendPushNotification = async (messages) => {
        try {
            if (messages.length > 0) {
                ...
                if (lastMessage.user._id !== currentUserRandomNumber) {
